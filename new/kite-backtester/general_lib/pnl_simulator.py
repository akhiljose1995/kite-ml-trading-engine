...
class PnLSimulator_OI_Tracker:
    ...
    def append_result_to_file(self, index_symbol, row_dict):
        try:
            filename = f"{index_symbol}_pnl_{datetime.now().date()}.csv"
    ...
    def track_trade(self, signal, end_time=None):
        try:
    ...
    def run(self):
        ...
        try:
            ...
            for signal in signals:
                ...
                self.track_trade(signal, end_time=end_time)
        except Exception:
            print("Error in run:\n", traceback.format_exc())

        """ def run(self):
        try:
            signal, prev_signal = None, None
            ...
            while not exit_flag:
                ...
                while True:
                    ...
                    prev_signal[end_time] = signal["datetime"] if signal else None
                ...
                if track_signal_flag:
                    ...
                    self.track_trade(prev_signal, end_time)
                    track_signal_flag = False
