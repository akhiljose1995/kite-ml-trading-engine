...
class PreprocessorTrain:
    """
    Handles full preprocessing for training data: cleaning, encoding, and scaling.
    Saves fitted encoders and scalers for later reuse.
    """
    ...
    def run(self, target: str) -> tuple:
        ...
        #directions = self.data_cleaner.unique_values_in_col(col="price_dir")
        ...
        df = self.scaler.fit_transform(df, numerical_col_list)
...
if __name__ == "__main__":
    processor = PreprocessorTrain(filepath="data/STATE_BANK_OF_INDIA.csv")
    ...
    y.to_frame(name="price_dir").to_csv("data/y_train.csv", index=False)  # Ensures y is saved as a column